...
import { deleteASubscriber, fetchSubscribersWithPaginate } from '../../api/api';
...
const SubsTable = () => {
    ...
    const [subscribersData, setSubscribersData] = useState<ISubscribersData[]>([])
    ...
    const confirm = async (id: string) => {
        const res: IBackendRes<any> = await deleteASubscriber(id);
        if (res && res.data) {
            message.success('Delete subscriber success')
            fetchSubs();
            return
        }
        else {
                ...
                return (
                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                        <Popconfirm
                            ...
                            description={`Are you sure to delete subscriber : ${record.email}`}
    ...
    const fetchSubs = async () => {
        ...
        let res: IBackendRes<IBackendResPaginate<ISubscribersData[]>> = await fetchSubscribersWithPaginate(query);
        if (res && res.data) {
            setCurrent(res.data.meta.current)
            setPageSize(res.data.meta.pageSize)
            setTotal(res.data.meta.total)
            setSubscribersData(res.data.result)
        }
        ...
        return (
            <div style={{ display: "flex", alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: "flex", alignItems: 'center', gap: '10px' }}>
                    ...
                    >
                        <UserAddOutlined />Add New Subscriber
                    </Button>
                </div>
            </div>
        )
